...
async def quality_from_csv(file: UploadFile = File(...)) -> QualityResponse:
    ...
    flags_all = compute_quality_flags(summary, missing_df, df)  # ← ЕДИНСТВЕННОЕ ИСПРАВЛЕНИЕ: правильный порядок аргументов
...
# ---------- НОВЫЙ ЭНДПОИНТ: /quality-flags-from-csv (обязательно по HW04) ----------

@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Получить только флаги качества по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, bool]]:
    ...
    Использует те же функции, что и CLI из HW03.
    Включает новые флаги: has_constant_columns, has_high_cardinality_categoricals и др.
    """
    if file.content_type not in ("text/csv", "application/vnd.ms-excel", "application/octet-stream"):
        raise HTTPException(status_code=400, detail="Ожидается CSV-файл (content-type text/csv).")
    ...
    # Только булевые флаги — как в примере задания
    flags_bool = {
        key: bool(value)
        for key, value in flags_all.items()
        if isinstance(value, bool)
