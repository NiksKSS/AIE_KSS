...
def compute_quality_flags(
    ...
    has_constant = (df.nunique() == 1).any()
    flags["has_constant_columns"] = bool(has_constant)

    # 2. Есть ли категориальные колонки с высокой кардинальностью?
    cat_cols = df.select_dtypes(include=["object", "category"]).columns
    has_high_card = False
    if len(cat_cols) > 0:
        unique_ratios = df[cat_cols].nunique() / len(df)
        has_high_card = (unique_ratios > 0.5).any()
    flags["has_high_cardinality_categoricals"] = bool(has_high_card)
